...
//  entry.data.pins = Pins;

  O.new('ose-control/lib/pin/list')(entry, Types, Pins, entry.data.dummy);
...
function setupGpio(pin, direction, edge, cb) {  // {{{2
  ...
  function setup() {  // {{{3
    try {
      ...
      if ((err.code === 'EACCES') && ((new Date().getTime() - start) < O.consts.gpioRetryTime)) {
        setTimeout(setup(), O.consts.gpioRetryTimeout);
        return;
      }
