...
exports.updateStateKey = function(key, value) {  // {{{2
  switch (key) {
  case 'last':
    displayImage(this, this.entry.sval.last);
    break;
  }
};

exports.displayLayout = function() {  // {{{2
  this.li().textButton('picture', 'Take picture', onTapStill.bind(this));

  this.li().append('img', 'still');
};

// }}}1
// Private {{{1
function onTapStill(ev) {  // {{{2
  this.post('still');
  return false;
};
...
function displayImage(that, id) {  // {{{2
  ...
  that.entry.findShard(that.entry.dval.save.shard, function(err, shard) {
    if (err) {
      O.log.error(err);
      return;
    }
