...
function setupPaths(entry) {  // {{{2
  if (! entry.dval.save.shard) {
    done(entry.dval.save.root, entry.dval.save.root);
    return;
  }

  entry.findShard(entry.dval.save.shard, function(err, shard) {
    if (err) {
      O.log.error(err);
      return;
    }
      ...
      (shard.db && shard.db.root) || entry.dval.save.root,
...
//    console.log('CAMERA ON READ', filename, Boolean(socket));
